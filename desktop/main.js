...
ipcMain.handle('compartments:create', (_event, profileId, input = {}) => { if (!profiles.has(profileId)) return null; const site = normalizeCompartment({ ...input, id: input.id || id(input.temporary ? 'temp' : 'site'), profileId }); compartments.set(site.id, site); saveDesktopStateSoon(); return clone(site); });
...
ipcMain.handle('compatibility:list', (_event, profileId) => [...compatibilityAllowances.values()].filter((allowance) => !profileId || allowance.profileId === profileId).map(clone));
